...
pub(crate) fn dlis(
    ...
    for (clause, _) in unsat_clauses {
        for &var in clause {
            let abs_var = var.abs();
            if free_vars.contains(&abs_var) || free_vars.contains(&-abs_var) {
    ...
    if max_var == 0 {
        return (*free_vars.iter().next().unwrap(), true);
    }
...
pub(crate) fn dlcs(
    ...
    for (clause, _) in unsat_clauses {
        for &var in clause {
            let abs_var = var.abs();
            if free_vars.contains(&abs_var) {
                *var_occurrences.entry(abs_var).or_insert(0) += 1;
            }
        }
    }

    let &max_var = var_occurrences.iter().max_by_key(|entry| entry.1).map(|(var, _)| var).unwrap_or(&0);
    if max_var == 0 {
        return (*free_vars.iter().next().unwrap(), true);
    }
...
pub(crate) fn mom(
    ...
    let min_clause_size = unsat_clauses.iter().map(|(_, size)| *size).min().unwrap_or(0);
    let mut var_occurrences: HashMap<i32, usize> = HashMap::new();

    for (clause, size) in unsat_clauses {
        if *size == min_clause_size {
            for &var in clause {
                if free_vars.contains(&var.abs()) {
...
pub(crate) fn boehm(
    ...
    for (clause, size) in unsat_clauses {
        if *size == min_clause_size {
            for &var in clause {
                if free_vars.contains(&var.abs()) {
                    *var_occurrences.entry(var.abs()).or_insert(0) += 2usize.pow(min_clause_size as u32); // Bias term
                }
            }
        }
    }
...
pub(crate) fn jeroslaw_wang(
    ...
    let &max_var = scores.iter().max_by(|a, b| a.1.partial_cmp(b.1).unwrap()).map(|(var, _)| var).unwrap_or(&0);
    if max_var == 0 {
        return (*free_vars.iter().next().unwrap(), true);
    }
